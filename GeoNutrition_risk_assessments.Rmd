...
## GeoNutrition prediction strategy (using stacking)
...
The core idea of stacking is to train multiple base models on the same training dataset and then use their predictions as input features for a second-level model, called the meta-model (stacked model). The stacked model is then trained to make the final prediction by combining the outputs of the base models (Breiman, 1996). This two-level prediction process can also be extended to multiple levels. The basic stacking process is as follows:
...
* Stacked model training: The stacked model is trained on the new set of features generated by the base model predictions, learning to combine them optimally.
